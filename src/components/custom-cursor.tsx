...
export default function SmoothCursor() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (cursorRef.current) {
        cursorRef.current.style.transform = `translate(${pos.current.x - 12}px, ${pos.current.y - 12}px) scale(${scale})`
      }
  ...
  return (
    <div
      ref={cursorRef}
      className="fixed top-0 left-0 w-6 h-6 rounded-full border border-white pointer-events-none z-[9999] transition-transform duration-200 ease-out"
